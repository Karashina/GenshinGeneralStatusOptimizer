...
def objective(x):
    ...
    return -(((106+weaponbase)*(1+a)+(min((15552*(1+h)*skillratio),(106+weaponbase)*4))*mvps)*(1+r*d)*(1+0.5*vaperatio+(4.17*vaperatio*e)/(e+1400)))

def constraint(x, StatsPool):
    # KQMC Rev-AのSub roll scalarに基づくステータス分布 
    a, h, r, d, e = x
    return a/0.0496 + h/0.0496 + r/0.0331 + d/0.0662 + e/19.82 - StatsPool
...
bounds = ((0, None), (0, None), (0, 1), (0.384, None), (0, None))
